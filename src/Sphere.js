...
function Sphere(props) {
	...
	const instanceHasMomentum = () => {
		return (posForce.x !== 0 || posForce.y !== 0 || posForce.z !== 0)
	}
	...
	useFrame((state, delta) => {
		...
		// if a non current sphere is moving according to db, then we want to give it momentum in the instance via posForce
		if (!(props.currentUserTrue) && !moving && dbHasMomentum()) {
			setPosForce({x: 1, y: 1, z: 1})
			// setPosForce(props.curPosForce)
			setMoving(true)
		} else if (!(props.currentUserTrue) && moving && !instanceHasMomentum()) {
			setMoving(false)
		}
		// bounding
		// const boundSize = 13
